...
from .job_contract import canonical, TERMINAL
...
class JobStore:
    ...
    def admit(self, client, request, request_hash):
        with self.lock, self.db:
            ...
            self.db.execute("INSERT INTO jobs VALUES(?,?,?,?,?,?,?,?,?,?,?)", (
                ...
                canonical(request), "accepted", now, now, None, None))
    ...
    def update(self, identifier, state, data=None, run_id=None):
        with self.lock, self.db:
            row = self.db.execute("SELECT state FROM jobs WHERE id=?", (identifier,)).fetchone()
            if row is None or row[0] in TERMINAL:
                return
            data = data or {}
            self.db.execute("UPDATE jobs SET state=?,updated=?,result=?,run_id=COALESCE(?,run_id) WHERE id=?",
                (state, time.time(), canonical(data) if state in TERMINAL else None, run_id, identifier))
    ...
    def recover(self):
        with self.lock:
            rows = self.db.execute("SELECT id FROM jobs WHERE state IN ('accepted','running','cancel_requested')").fetchall()
        for row in rows:
            self.update(row[0], "unknown", {"reason": "service_restarted_without_confirmed_settlement", "local_stopped": None, "remote_stopped": None, "applied": False})
